...
@foreach ($list as $item)
    ...
    <ul>
        <li>
            <a href="{{ route('detalhe', ['title' => str_slug($item->news[0]->title), 'id' => $item->news[0]->id]) }}">
            ...
            <div class="viewLine clearfix">
                ...
                @if (count($item->news) > 0)
                    ...
                    @for ($i = 0; $i < count($item->news) && $i < 5; $i++)
                        <a href="{{ route('detalhe', ['title' => str_slug($item->news[$i]->title), 'id' => $item->news[$i]->id]) }}" class="boxVehicle">{{ $item->news[$i]->vehicle->description }}</a>
                    @endfor
            ...
            @if (count($item->olds) > 0)
                <div class="viewLine previous clearfix">
                    ...
                    @for ($i = 0; $i < count($item->olds) && $i < 5; $i++)
                        <a href="{{ route('detalhe', ['title' => str_slug($item->olds[$i]->title), 'id' => $item->olds[$i]->id]) }}" class="boxVehicle">{{ $item->olds[$i]->vehicle->description }}</a>
                    @endfor
