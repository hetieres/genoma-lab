    ...
    <main class="container internal">
        <h1>
            ...
            <ul class="breadcrumbs">
                ...
                <li class="active"><span>Veículos Nacionais</span></li>
            </ul>
        </h1>
        ...
        <div class="col-12 col-sm-8 col-md-9 no-padding-left no-padding-xs">
            ...
            <ul class="pagination" role="navigation">
                @if ($lastPage > 5)
                    <li class="previous"><a href="{{ route('vehicles', ['page' => ($currentPage - 1 > 0 ? $currentPage - 1 : 1)]) }}"><i class="fa fa-chevron-left"></i> Anterior</a></li>
                @endif

                @for ($i = 0; $i < count($rangePages); $i++)
                    @if ($currentPage !==  $rangePages[$i])
                        <li class="page-item">
                            <a class="page-link" href="{{ route('vehicles', ['page' => $rangePages[$i]]) }}">
                                {{ $rangePages[$i] }}
                            </a>
                        </li>
                    @else
                ...
                @if ($lastPage > 5)
                    <li class="next"><a href="{{ route('vehicles', ['page' => ($currentPage + 1 > $lastPage ? $lastPage : $currentPage + 1)]) }}">Próximo <i class="fa fa-chevron-right"></i></a></li>
                @endif
            </ul>
