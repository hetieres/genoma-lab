    ...
    <main class="container internal">
        <h1>
            ...
            <ul class="breadcrumbs">
                ...
                <li><a href="{{ route('home') }}">país</a></li>
                <li class="active"><span>{{ $country->description }}</span></li>
            </ul>
        </h1>
        ...
        <div class="col-12 col-sm-8 col-md-9 no-padding-left no-padding-xs">
            <div class="vehiclesHighlights">
                <div class="boxTitle">
                    ...
                    <div class="total">
                        <h5><b>Em {{ $year }}:</b> {{ $country->total_year }} notícias</h5>
                        <h5><b>Desde 1995:</b> {{ $country->total }} notícias</h5>
                    </div>
                </div>
            ...
            @if ($lastPage > 1)
                <ul class="pagination" role="navigation">
                    @if ($lastPage > 5)
                        <li class="previous"><a href="{{ route('country', ['description' => str_slug($country->description), 'id' => $country->id, 'page' => ($currentPage - 1 > 0 ? $currentPage - 1 : 1)]) }}"><i class="fa fa-chevron-left"></i> Anterior</a></li>
                    @endif

                    @for ($i = 0; $i < count($rangePages); $i++)
                        @if ($currentPage !=  $rangePages[$i])
                            <li class="page-item">
                                <a class="page-link" href="{{ route('country', ['description' => str_slug($country->description), 'id' => $country->id, 'page' => $rangePages[$i]]) }}">
                                    {{ $rangePages[$i] }}
                                </a>
                            </li>
                        @else
                    ...
                    @if ($lastPage > 5)
                        <li class="next"><a href="{{ route('country', ['description' => str_slug($country->description), 'id' => $country->id, 'page' => ($currentPage + 1 > $lastPage ? $lastPage : $currentPage + 1)]) }}">Próximo <i class="fa fa-chevron-right"></i></a></li>
                    @endif
                </ul>
            @endif
        </div>
